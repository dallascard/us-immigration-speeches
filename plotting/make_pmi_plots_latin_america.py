...
# Variation of plotting/make_pmi_plots.py, which does comparison between Europeans and Latin America (for figure in SI)
...
def main():
    ...
    #tones = sorted(imm_token_counts_by_congress_by_tone)
    #imm_total_tokens_by_congress_by_tone = {}
    #for tone in tones:
    #    imm_total_tokens_by_congress_by_tone[tone] = {congress: sum(imm_token_counts_by_congress_by_tone[tone][congress].values()) for congress in congresses}
    ...
    modern_eu_log_probs = []
    for c in range(modern_start, modern_end+1):
        modern_me_counts.update(imm_sent_token_counts_by_congress_by_group['Hispanic'][c])
        ...
        modern_counts.update(token_counts_by_congress[c])
        modern_me_log_probs.extend(metaphor_log_probs_by_congress_by_group['Hispanic']['combined'][str(c)])
    ...
    for f_i, frame in enumerate(frame_order):
        ...
        overall_count = 0
        for term in terms:
            #print(term, modern_chinese_lemmas[term], modern_europe_lemmas[term])
            me_count += modern_me_counts[term]
            eu_count += modern_eu_counts[term]
    ...
    fig, ax = plt.subplots(figsize=(3, 7), sharey=False, sharex=True)
    ...
    factor = (xlim - log_ratio) / xlim / 2
    color = list(np.array([0, 0, 1]) * (factor) + np.array([1, 0, 0]) * (1-factor))
    ax.scatter([log_ratio],  0,  color=color, marker='x')

    combined = []
    denom = []
    for i, m in enumerate(modern_log_means):
        combined.append(m * modern_freqs[i])
        denom.append(modern_freqs[i])
    ...
    for i, f_i in enumerate(order):
        ...
        ax.plot([modern_lowers[f_i], modern_uppers[f_i]], [n_frames-i, n_frames-i], color=color, linewidth=1.5)
        ax.scatter([log_ratio], [n_frames-i], color=color, s=max(2, modern_freq_ratios[f_i] * size_factor))

    ax.set_yticks(range(0,n_frames+1))
    ax.set_yticks(range(0,n_frames+1))
    ax.set_yticklabels([t.title() for t in labels[::-1]], fontsize=14)
    ax.set_xticks([-2, -1, 0, 1, 2])
    ax.set_xticklabels(['2', '-1', '0', '1', '2'], fontsize=14)
    ax.plot([0, 0], [-0.95, n_frames + 0.95], 'k--', alpha=0.5)
    ax.set_ylim(-0.95, 14.95)
    ax.set_xlim(-2.6, 2.6)

    ax.set_title('(2001-2020)\n' + r'European $\Longleftrightarrow$ Latin American')
    ax.set_xlabel('log(Frequency ratio)')

    if do_query_expansion:
        plt.savefig(os.path.join(outdir, 'PMI_early_vs_modern_by_group_mod2.pdf'), bbox_inches='tight')
        plt.savefig(os.path.join(outdir, 'PMI_early_vs_modern_by_group_mod2.png'), bbox_inches='tight')
    else:
        plt.savefig(os.path.join(outdir, 'PMI_early_vs_modern_by_group2.pdf'), bbox_inches='tight')
        plt.savefig(os.path.join(outdir, 'PMI_early_vs_modern_by_group2.png'), bbox_inches='tight')
...
def expand_frame_terms(tagged_frame_terms, vector_file, overall_tagged_counts, exp_count=20):
    ...
    #frame_word_frames = defaultdict(list)
    #for frame, words in frame_words_simple.items():
    #    for word in words:
    #        frame_word_frames[word].append(frame)
    #print(frame_word_frames['illegal'])
